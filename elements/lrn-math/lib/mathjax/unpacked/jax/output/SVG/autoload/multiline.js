...
MathJax.Hub.Register.StartupHook("SVG Jax Ready", function() {
  ...
  MML.mbase.Augment({
    ...
    SVGbetterBreak: function(info, state, toplevel) {
      ...
      while (
        i < m &&
        (info.scanW < PENALTY.maxwidth * SVG.linebreakWidth || info.w === 0)
      ) {
        if (this.data[i]) {
  ...
  MML.mfenced.Augment({
    ...
    SVGmoveLine: function(start, end, svg, state, values) {
      ...
      if (i === j && start.length > 1) {
        ...
        //  If starting and ending in the same element move the subpiece to the new line
        //
        this.data[this.dataI[i]].SVGmoveSlice(
          start.slice(1),
          end.slice(1),
          svg,
          state,
  ...
  MML.mmultiscripts.Augment({
    SVGbetterBreak: function(info, state) {
      ...
      //
      var dw =
        this.SVGdata.w - this.data[this.base].SVGdata.w - this.SVGdata.dx;
      //
  ...
  MML.mo.Augment({
    ...
    SVGbetterBreak: function(info, state) {
      ...
      info.W = W;
      info.w = w;
      values.lineleading = SVG.length2em(
        values.lineleading,
        1,
  ...
  MML.semantics.Augment({
    ...
    SVGmoveLine: function(start, end, svg, state, values) {
      return this.data[0]
        ? this.data[0].SVGmoveSlice(start, end, svg, state, values)
        : null;
    }
  });
