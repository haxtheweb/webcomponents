...
MathJax.Hub.Register.StartupHook("HTML-CSS Jax Ready", function() {
  ...
  MML.mbase.Augment({
    ...
    HTMLmultiline: function(span) {
      ...
      while (
        parent.inferred ||
        (parent.parent &&
          parent.parent.type === "mrow" &&
          parent.isEmbellished())
      ) {
        parent = parent.parent;
      }
      var isTop =
        (parent.type === "math" && parent.Get("display") === "block") ||
        parent.type === "mtd";
    ...
    HTMLbetterBreak: function(info, state, toplevel) {
      ...
      while (
        i < m &&
        (info.scanW < PENALTY.maxwidth * HTMLCSS.linebreakWidth || info.w === 0)
      ) {
        if (this.data[i]) {
    ...
    HTMLaddLine: function(stack, start, end, state, values, broken) {
      ...
      if (state.n > 0) {
        ...
        var leading =
          (state.values.lineleading == null ? state.VALUES : state.values)
    ...
    HTMLgetAlign: function(state, values) {
      ...
      if (state.n === 0) {
        align =
          cur.indentalignfirst || prev.indentalignfirst || def.indentalignfirst;
    ...
    HTMLgetShift: function(state, values, align) {
      ...
      if (state.n === 0) {
        shift =
          cur.indentshiftfirst || prev.indentshiftfirst || def.indentshiftfirst;
      } else if (state.isLast) {
  ...
  MML.mfenced.Augment({
    ...
    HTMLmoveLine: function(start, end, span, state, values) {
      ...
      if (i === j && start.length > 1) {
        ...
        //  If starting and ending in the same element move the subpiece to the new line
        //
        this.data[this.dataI[i]].HTMLmoveSlice(
          start.slice(1),
          end.slice(1),
          span,
          state,
      ...
      if (end.length === 0) {
        ...
        if (s && this.HTMLnotEmpty(s)) {
          ...
          for (box = stack.firstChild; box; box = box.nextSibling) {
            box.style.left = HTMLCSS.Em(
              HTMLCSS.unEm(box.style.left) - this.HTMLbaseW
            );
          }
      ...
      if (start.length < 1) {
        if (BOX.presub || BOX.presup) {
          ...
          if (BOX.presub) {
            ...
            HTMLCSS.placeBox(
              BOX.presub,
              data.dx + data.delta - BOX.presub.bbox.w,
              -data.v
            );
          }
          this.HTMLcombineBBoxes(STACK, span.bbox);
  ...
  MML.mo.Augment({
    ...
    HTMLbetterBreak: function(info, state) {
      ...
      info.W = W;
      info.w = w;
      values.lineleading = HTMLCSS.length2em(
        values.lineleading,
        1,
  ...
  MML.maction.Augment({
    ...
    //
    HTMLmoveSlice: function(start, end, span, state, values, padding) {
      var hitbox = document.getElementById(
        "MathJax-HitBox-" + this.spanID + HTMLCSS.idPostfix
      );
      if (hitbox) {
  ...
  MML.semantics.Augment({
    ...
    HTMLmoveLine: function(start, end, span, state, values) {
      return this.data[0]
        ? this.data[0].HTMLmoveSlice(start, end, span, state, values)
        : null;
    }
  });
