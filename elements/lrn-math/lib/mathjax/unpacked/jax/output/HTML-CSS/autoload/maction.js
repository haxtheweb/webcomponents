...
MathJax.Hub.Register.StartupHook("HTML-CSS Jax Ready", function() {
  ...
  MML.maction.Augment({
    HTMLtooltip: HTMLCSS.addElement(document.body, "div", {
      id: "MathJax_Tooltip"
    }),
    ...
    HTMLhandleHitBox: function(span, postfix) {
      ...
      if (HTMLCSS.msieHitBoxBug) {
        ...
        if (HTMLCSS.msieInlineBlockAlignBug) {
          frame.style.verticalAlign = HTMLCSS.Em(
            HTMLCSS.getHD(span).d - span.bbox.d
          );
        }
      } else {
        frame = HTMLCSS.createFrame(
      ...
      frame.id =
        "MathJax-HitBox-" + this.spanID + (postfix || "") + HTMLCSS.idPostfix;
    ...
    HTMLtooltipPost: function(x, y) {
      ...
      //
      var emex = tip.insertBefore(
        HTMLCSS.TestSpan.cloneNode(true),
        tip.firstChild
      );
      ...
      var scale = Math.floor(
        Math.max(
          HTMLCSS.config.minScaleAdjust / 100,
          ex / HTMLCSS.TeX.x_height / em
    ...
    HTMLtooltipClear: function(n) {
      ...
      if (n <= 0) {
        ...
        clear = setTimeout(
          MathJax.Callback(["HTMLtooltipClear", this, n - 20]),
          50
        );
      }
    }
