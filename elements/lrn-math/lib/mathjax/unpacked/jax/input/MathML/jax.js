...
(function(MATHML, BROWSER) {
  ...
  MATHML.Parse = MathJax.Object.Subclass(
    {
      ...
      Parse: function(math, script) {
        ...
        if (doc.firstChild.nodeName.toLowerCase() === "html") {
          ...
          if (h1 && h1.textContent === "XML parsing error" && h1.nextSibling)
            MATHML.Error([
              "ParsingError",
              "Error parsing MathML: %1",
              String(h1.nextSibling.nodeValue).replace(
                /fatal parsing error: /,
                ""
              )
            ]);
        ...
        if (
          doc.firstChild.nodeName.toLowerCase().replace(/^[a-z]+:/, "") !==
          "math"
        ) {
          MATHML.Error([
            "MathMLRootElement",
      ...
      MakeMML: function(node) {
        ...
        if (match) {
          mml = this.TeXAtom(
            match[2],
            match[2] === "OP" && !CLASS.match(/MJX-fixedlimits/)
          );
            ...
            if (CLASS[i] === "MJX-arrow") {
              ...
              if (!MathJax.Extension["TeX/AMSsymbols"]) {
                MathJax.Hub.RestartAfter(
                  MathJax.Ajax.Require("[MathJax]/extensions/TeX/AMSsymbols.js")
                );
              }
              ...
              //
              if (
                mml.mathvariant === "-tex-caligraphic-bold" ||
      ...
      AddAttributes: function(mml, node) {
        ...
        for (var i = 0, m = node.attributes.length; i < m; i++) {
          ...
          var defaults =
            mml.type === "mstyle" ? MML.math.prototype.defaults : mml.defaults;
          if (value != null) {
            ...
            if (val === "true" || val === "false") {
              if (
                ...
                (defaults[name] === MML.AUTO &&
                  (mml.defaultDef == null ||
                    typeof mml.defaultDef[name] === "boolean"))
              ) {
                value = val === "true";
              }
            }
      ...
      replaceEntity: function(match, entity) {
        ...
        if (!MATHML.Parse.loaded[file]) {
          ...
          MathJax.Hub.RestartAfter(
            MathJax.Ajax.Require(MATHML.entityDir + "/" + file + ".js")
          );
        }
        return match;
      }
  ...
  MATHML.Augment({
    ...
    Translate: function(script) {
      ...
      if (
        script.firstChild &&
        script.firstChild.nodeName.toLowerCase().replace(/^[a-z]+:/, "") ===
          "math"
      ) {
