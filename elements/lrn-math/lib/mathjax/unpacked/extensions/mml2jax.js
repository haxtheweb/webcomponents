    ...
    if (typeof document.namespaces !== "undefined") {
      ...
      if (html) {
        for (i = 0, m = html.attributes.length; i < m; i++) {
          ...
          if (
            attr.nodeName.substr(0, 6) === "xmlns:" &&
            attr.nodeValue === this.MMLnamespace
          ) {
            this.PushMathElements(
              mathArray,
              element,
              attr.nodeName.substr(6) + ":math"
            );
          }
        }
      }
  ...
  NodeHTML: function(node) {
    ...
    if (node.nodeName === "#text") {
      ...
      for (i = 0, m = node.attributes.length; i < m; i++) {
        ...
        if (
          attribute.specified &&
          attribute.nodeName.substr(0, 10) !== "_moz-math-"
        ) {
          // Opera 11.5 beta turns xmlns into xmlns:xmlns, so put it back (*** check after 11.5 is out ***)
          html +=
            " " +
            attribute.nodeName.toLowerCase().replace(/xmlns:xmlns/, "xmlns") +
            "=";
