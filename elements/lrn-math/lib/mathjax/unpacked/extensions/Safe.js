...
(function(HUB, AJAX) {
  ...
  var SAFE = (MathJax.Extension.Safe = {
    ...
    filterStyleLength: function(name, value, styles) {
      if (typeof CONFIG.styleLengths[name] === "string")
        value = styles[CONFIG.styleLengths[name]];
      value = this.length2em(value);
      if (value == null) return false;
      var mM = [-CONFIG.lengthMax, CONFIG.lengthMax];
      if (MathJax.Object.isArray(CONFIG.styleLengths[name]))
        mM = CONFIG.styleLengths[name];
    ...
    PARSE.Augment({
      ...
      SetSize: function(name, size) {
        ...
        if (size) {
          ...
          this.Push(
            TEX.Stack.Item.style().With({ styles: { mathsize: size + "em" } })
          );
        }
      }
    });
