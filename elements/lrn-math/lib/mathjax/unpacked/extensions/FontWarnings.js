...
(function(HUB, HTML) {
  ...
  var CONFIG = HUB.CombineConfig("FontWarnings", {
    ...
    messageStyle: {
      ...
      filter:
        "progid:DXImageTransform.Microsoft.dropshadow(OffX=3, OffY=3, Color='gray', Positive='true')" // IE
    },
  ...
  var CREATEMESSAGE = function(data) {
    ...
    while (i < data.length) {
      if (MathJax.Object.isArray(data[i])) {
        if (data[i].length === 1 && CONFIG.HTML[data[i][0]]) {
          ...
          var message = MathJax.Localization.lookupPhrase(
            ["FontWarnings", data[i][0]],
            data[i][1]
          );
          message = MathJax.Localization.processMarkdown(
            message,
            data[i].slice(2),
            "FontWarnings"
          );
    ...
    DATA.div = HTMLCSS.addElement(
      frame,
      "div",
      { id: "MathJax_FontWarning", style: CONFIG.messageStyle },
      data
    );
  ...
  var FADEOUT = function() {
    ...
    if (DATA.fade < CONFIG.fadeoutSteps) {
      ...
      DATA.div.style.filter =
        "alpha(opacity=" + Math.floor(100 * opacity) + ")";
    ...
    HUB.Startup.signal.Interest(function(message) {
      if (message.match(/HTML-CSS Jax - /) && !DATA.div) {
        ...
        if (MSG && CONFIG.Message[MSG]) {
          MathJax.Localization.loadDomain("FontWarnings", [
            CREATEMESSAGE,
            CONFIG.Message[MSG]
          ]);
        }
