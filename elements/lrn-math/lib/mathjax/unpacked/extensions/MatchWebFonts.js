  ...
  HUB.Register.StartupHook("HTML-CSS Jax Ready", function() {
    ...
    HTMLCSS.Augment({
      postTranslate: function(state, partial) {
        if (
          !partial &&
          CONFIG.matchFor["HTML-CSS"] &&
          this.config.matchFontHeight
        ) {
          //
    ...
    nMML.Augment({
      ...
      checkFonts: function(check, scripts) {
        ...
        for (i = 0, m = scripts.length; i < m; i++) {
          ...
          if (
            "scrollWidth" in jax &&
            (newEx || jax.scrollWidth !== math.firstChild.scrollWidth)
          ) {
            jax.scrollWidth = math.firstChild.scrollWidth;
            adjust.push([
              math.parentNode.style,
              jax.scrollWidth / jax.ex / jax.scale
            ]);
          }
          //
          ...
          if (math.MathJaxMtds) {
            for (var j = 0, n = math.MathJaxMtds.length; j < n; j++) {
              ...
              if (
                newEx ||
                math.MathJaxMtds[j].firstChild.scrollWidth !== jax.mtds[j]
              ) {
