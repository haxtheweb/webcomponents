...
(function(HUB, HTML, AJAX, CALLBACK, LOCALE, OUTPUT, INPUT) {
  ...
  var EVENT = (ME.Event = {
    ...
    ContextMenu: function(event, math, force) {
      ...
      var show = (JAX.config.showMathMenu != null ? JAX : HUB).config
        .showMathMenu;
      ...
      if (MENU) {
        ...
        if (!load) {
          ...
          MathPlayer.hidden = !(
            jax.outputJax === "NativeMML" && HUB.Browser.hasMathPlayer
          );
    ...
    AltContextMenu: function(event, math) {
      ...
      var show = (JAX.config.showMathMenu != null ? JAX : HUB).config
        .showMathMenu;
      if (show) {
        show = (JAX.config.showMathMenuMSIE != null ? JAX : HUB).config
          .showMathMenuMSIE;
  ...
  var HOVER = (ME.Hover = {
    ...
    Mouseover: function(event, math) {
      if (SETTINGS.discoverable || SETTINGS.zoom === "Hover") {
        ...
        if (
          from &&
          to &&
          (HUB.isMathJaxNode(from) !== HUB.isMathJaxNode(to) ||
    ...
    Hover: function(jax, math) {
      ...
      var JAX = OUTPUT[jax.outputJax],
        ...
        show = (JAX.config.showMathMenu != null ? JAX : HUB).config
          .showMathMenu;
        ...
        [
          [
            "span",
            {
              ...
              style: {
                left: this.Px(
                  bbox.w + dx + dd + (bbox.x || 0) + CONFIG.button.x
                ),
                top: this.Px(
                  -bbox.h - dy - dd - (bbox.y || 0) - CONFIG.button.y
                ),
                opacity: 0,
                filter: "alpha(opacity=0)"
              }
    ...
    ReHover: function(jax) {
      ...
      jax.hover.remove = setTimeout(
        CALLBACK(["UnHover", this, jax]),
        CONFIG.fadeoutDelay
      );
    ...
    HoverFade: function(jax) {
      ...
      jax.hover.opacity = Math.max(
        0,
        Math.min(1, jax.hover.opacity + jax.hover.inc)
      );
      jax.hover.opacity = Math.floor(1000 * jax.hover.opacity) / 1000;
      var frame = document.getElementById(jax.hover.id),
        button = document.getElementById(jax.hover.id + "Menu");
      frame.firstChild.style.opacity = jax.hover.opacity;
      frame.firstChild.style.filter =
        "alpha(opacity=" + Math.floor(100 * jax.hover.opacity) + ")";
      if (button) {
    ...
    HoverFadeTimer: function(jax, inc, delay) {
      jax.hover.inc = inc;
      if (!jax.hover.timer) {
        jax.hover.timer = setTimeout(
          CALLBACK(["HoverFade", this, jax]),
  ...
  var SETFRAME = function() {
    ...
    haze.border =
      CONFIG.frame.bwidth + "px solid " + CONFIG.frame.bcolor + " ! important";
    haze["box-shadow"] = haze["-webkit-box-shadow"] = haze[
      "-moz-box-shadow"
    ] = haze["-khtml-box-shadow"] =
