define([
  ...
  _resolveUrl,
  _paperProgress,
  _lrnsysProgressCircle
) {
  "use strict";
  Object.defineProperty(_exports, "__esModule", { value: !0 });
  _exports.LrnsysProgress = void 0;
  meta = babelHelpers.interopRequireWildcard(meta);
  function _templateObject_8afa7db0f32d11e8a7ce37a700b95e80() {
    ...
    _templateObject_8afa7db0f32d11e8a7ce37a700b95e80 = function _templateObject_8afa7db0f32d11e8a7ce37a700b95e80() {
      return data;
    };
    return data;
  }
  var LrnsysProgress = (0, _polymerLegacy.Polymer)({
    _template: (0, _polymerLegacy.html)(
      _templateObject_8afa7db0f32d11e8a7ce37a700b95e80()
    ),
    is: "lrnsys-progress",
    listeners: {
    ...
    properties: {
      ...
      completeSound: {
        type: String,
        value:
          (0, _resolveUrl.pathFromUrl)(meta.url) + "lib/assets/complete.mp3",
        reflectToAttribute: !0
      },
      finishedSound: {
        type: String,
        value:
          (0, _resolveUrl.pathFromUrl)(meta.url) + "lib/assets/finished.mp3",
        reflectToAttribute: !0
      },
      title: {
        type: String,
    ...
    _getStrokeWidth: function _getStrokeWidth(size) {
      var width = 4;
      if ("tiny" == size) {
        width = 3;
      } else if ("small" == size) {
    ...
    _itemsChanged: function _itemsChanged(newValue, oldValue) {
      ...
      if (
        ...
        if (
          ...
          setTimeout(function() {
            newValue[_this.active].status = "available";
            _this.set("items." + _this.active + ".status", "available");
            _this.notifyPath("items." + _this.active + ".status");
    ...
    _isActive: function _isActive(index, active) {
      return index === active;
    },
    _activeResponseChange: function _activeResponseChange(value) {
    ...
    _activeChanged: function _activeChanged(newValue, oldValue) {
      ...
      this.items.forEach(function(element, index, array) {
        if ("disabled" == _this2.items[index].status) {
          if (
            0 != index &&
            _this2.progressiveUnlock &&
            "complete" == _this2.items[index - 1].status
          ) {
            _this2.items[index].status = "loading";
            _this2.set("items." + index + ".status", "loading");
            _this2.notifyPath("items." + index + ".status");
          }
        } else if (
          _this2.items[index].value >= _this2.items[index].max &&
          index == _this2.items.length - 1
        ) {
          _this2.items[index].status = "finished";
          _this2.set("items." + index + ".status", "finished");
          _this2.notifyPath("items." + index + ".status");
        } else if (_this2.items[index].value >= _this2.items[index].max) {
          _this2.items[index].status = "complete";
          ...
          if (
            ...
            _this2.items[index].status = "available";
    ...
    _statusChanged: function _statusChanged(e) {
      ...
      if ("loading" == e.target.status) {
        if (
          ...
          setTimeout(function() {
            _this3.items[_this3.active].status = "available";
            _this3.set("items." + _this3.active + ".status", "available");
            _this3.notifyPath("items." + _this3.active + ".status");
        ...
        setTimeout(function() {
          _this3.items[_this3.active].status = "finished";
          _this3.set("items." + _this3.active + ".status", "finished");
          _this3.notifyPath("items." + _this3.active + ".status");
        }, 100);
      }
    },
    handleNodeResponse: function handleNodeResponse(e) {
      var _this4 = this,
        detail = e.detail;
      if (babelHelpers.typeof(detail.response) === babelHelpers.typeof(null)) {
        setTimeout(function() {
          _this4.items[_this4.active].status = "available";
          _this4.set("items." + _this4.active + ".status", "available");
          _this4.notifyPath("items." + _this4.active + ".status");
    ...
    _overallPercentageCompute: function _overallPercentageCompute(
      items,
      active
    ) {
    ...
    changePercentage: function changePercentage(percentage, mode) {
      var newp = 0;
      if ("add" == mode) {
    ...
    updateItems: function updateItems(op, item) {
      var response = !1;
      if ("push" == op) {
