      ...
      <product-card
        ...
        <div class="switch">
          <mwc-switch
            ...
            data-file="${el.file}"
            checked
            @change="${this.elementStatusChange}"></mwc-switch>
            <label for="switch-${i}">${el.status ? html`Enabled` : html`Disabled`}</label>
        </div>
        ...
        <div slot="details-collapse-content" class="card-content">
          <ul>
            ...
            <li><strong>Tags:</strong> ${el.schema.gizmo.groups.map((group) => html`${group}, `)}</li>
        ${el.schema.gizmo.meta ? html`
          ${Object.keys(el.schema.gizmo.meta).map((mel) => html`
            <li><strong>${this.capFirst(mel)}:</strong> <span>${el.schema.gizmo.meta[mel]}</span></li>
          `)}
        ` : ``}
          </ul>
        </div>
        ...
        <div slot="demo-collapse-content">
          ${el.schema.demoSchema ? html`${this._haxElementToNode(el.schema.demoSchema)}
