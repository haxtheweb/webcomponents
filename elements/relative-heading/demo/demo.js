  ...
  let changer = document.getElementById("changer"),
    ...
    randomHeading = () => {
      let h = Math.floor(Math.random() * Math.floor(6)),
        options = ['aliquam','justo','nec','iaculis','viverra','dignissim'],
        text = options[Math.floor(Math.random() * Math.floor(5))], 
        tag = h > 0 ? `h${h}` : 'p';
      return `<${tag}>Praesent ${text}</${tag}>`;
    };

  changer.onchange = (e) => {console.log('change',sapien,sapien.parent,changer.value); sapien.parent = changer.value; console.log('change 2',sapien,sapien.parent,changer.value);}
