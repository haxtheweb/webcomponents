  ...
  renderStar(amount, interactive) {
    ...
    while (count < amount) {
      if ((count+1) < (amount * this._calPercent)) {
        content.push(html`<simple-icon-button icon="star" dark contrast="1" class='star' ?disabled="${!interactive}"></simple-icon-button>`);
      }
      else if (((amount * this._calPercent) - (count+1)) < -0.01 && ((amount * this._calPercent) - (count+1)) >= -0.5) {
        content.push(html`<simple-icon-button icon="star-half" dark contrast="1" class='star' ?disabled="${!interactive}"></simple-icon-button>`);
      }
      else {
        content.push(html`<simple-icon-button icon="star-border" dark contrast="1" class='star' ?disabled="${!interactive}"></simple-icon-button>`);
      }
      count++;
    }
    return content;
