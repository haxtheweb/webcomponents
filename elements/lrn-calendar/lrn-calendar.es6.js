...
*/let LrnCalendar=Polymer({_template:html`
  ...
  `,is:"lrn-calendar",behaviors:[HAXBehaviors.PropertiesBehaviors],properties:{/**
   ...
   */_fileChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&this.__icalLoaded){this.loadFile();this.getDateInfo();this.createCalendar()}},/**
   * Notice the file has changed and attempt to rebind.
   */_viewTypeChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&typeof this.file!==typeof void 0&&this.__icalLoaded){this.getDateInfo();this.createCalendar()}},/**
   * Notice the file has changed and attempt to rebind.
   */_dateChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&typeof this.file!==typeof void 0&&this.__icalLoaded){this.getDateInfo();this.createCalendar()}},/**
   * Notice the String of the date has changed and reset the actual date to it
   */_dateStringChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&""!=newValue&&this.__icalLoaded){this.date=new Date(newValue)}},/**
   * created life cycle
   */created:function(){const name="ical",basePath=pathFromUrl(decodeURIComponent(import.meta.url)),location=`${basePath}lib/ical.js/build/ical.js`;window.addEventListener(`es-bridge-${name}-loaded`,this._icalLoaded.bind(this));window.ESGlobalBridge.requestAvailability();window.ESGlobalBridge.instance.load(name,location)},/**
   * ical loaded
   */_icalLoaded:function(){this.__icalLoaded=!0;if(typeof this.file!==typeof void 0){this.loadFile()}},/**
   * Attached lifecycle.
   */attached:function(){// Establish hax properties if they exist
//this._initializeReader();
let props={canScale:!1,canPosition:!1,canEditSource:!1,gizmo:{title:"Calendar",description:"Present dates.",icon:"icons:today",color:"grey",groups:["Date"],handles:[{type:"ical",source:"file",date:"date"}],meta:{author:"LRNWebComponents"}},settings:{quick:[{property:"file",title:"iCal feed",description:"Calendar feed to display",inputMethod:"textfield",icon:"link",required:!0}],configure:[{property:"file",title:"iCal feed",description:"Calendar feed to display",inputMethod:"textfield",icon:"link",required:!0,validationType:"url"},{property:"view",title:"View calendar in week or month format",description:"week or month view",inputMethod:"select",options:{week:"Week",month:"Month"}},{property:"dateString",title:"Date: yyyy/mm/dd",description:"Date to display calendar",inputMethod:"datepicker"}],advanced:[]}};this.setHaxProperties(props)},/**
   * Load the ical file.
   */loadFile:function(){this.startIndex=0;this.calendarText=this.readTextFile(this.file);this.listOfEvents=this.getEvents(this.date,this.calendarText);this.listOfEvents=this.sortByTime(this.listOfEvents)},/*
   * Is called to show the previous month/week depending on option selected
   */showPrev:function(){var current;this.startIndex=0;if("month"==this.view){if(2==this.date.getMonth()){if(28<this.date.getDate()){current=new Date(this.date.getFullYear(),this.date.getMonth()-1,28)}else{current=new Date(this.date.getFullYear(),this.date.getMonth()-1,this.date.getDate())}}else{current=new Date(this.date.getFullYear(),this.date.getMonth()-1,this.date.getDate())}this.date=current}else{var current=new Date(this.date.getFullYear(),this.date.getMonth(),this.date.getDate()-7);this.date=current}this.startMonth=this.date.getMonth()+1;this.getDateInfo();this.createCalendar()},/*
   * Is called to show the next month/week
   */showNext:function(){var current;if("month"==this.view){if(11==this.date.getMonth()){current=new Date(this.date.getFullYear()+1,0,this.date.getDate())}else if(0==this.date.getMonth()){if(28<this.date.getDate()){current=new Date(this.date.getFullYear(),this.date.getMonth()+1,28)}else{current=new Date(this.date.getFullYear(),this.date.getMonth()+1,this.date.getDate())}}else{current=new Date(this.date.getFullYear(),this.date.getMonth()+1,this.date.getDate())}this.date=current}else{var current=new Date(this.date.getFullYear(),this.date.getMonth(),this.date.getDate()+7);this.date=current}this.getDateInfo();this.createCalendar()},/**
   * Is called to show the next month/week
   */showDate:function(){this.startIndex=0;this.date=new Date;this.getDateInfo();this.createCalendar()},//Set the view to month
monthView:function(){this.view="month";this.startIndex=0;this.createCalendar()},//Set the view to weekly
weekView:function(){this.view="week";this.startIndex=0;this.createCalendar()},readTextFile:function(file){this.fileNotFound=!1;var rawFile=new XMLHttpRequest,allText="";rawFile.open("GET",file,!1);rawFile.onreadystatechange=function(){if(4===rawFile.readyState){if(200===rawFile.status||0==rawFile.status){allText=rawFile.responseText}else{allText="Not Found"}}};rawFile.send(null);this.calendarText=allText;return allText},/*
   ...
   */createCalendar:function(){if("month"!=this.view){if("week"!=this.view){this.view="month"}}if(!this.date.getFullYear()){this.date=new Date}this.getDateInfo();this.totalDays=7*this.numweeks;this.calendarText=this.readTextFile(this.file);this.calendarView=this.$.calView;var days=1,pastDate=0,elem=dom(this.$.calView).node,elemChildren=elem.childNodes;while(elemChildren[1]){elem.removeChild(elem.childNodes[1])}//*******************************************************************
//Setup for calendar*******************************************************************
...
if(12==this.currentMonth&&31==this.currentDate){this.currentYear=this.currentYear+1;this.currentMonth=1;this.currentDate=1;this.newDay=new Date(this.currentYear+"/"+this.currentMonth+"/"+this.currentDate)}else{this.currentDate=this.currentDate+1;this.newDay=new Date(this.currentYear+"/"+this.currentMonth+"/"+this.currentDate);if(this.newDay.getDate()!=this.currentDate){if(1==this.currentMonth||3==this.currentMonth||5==this.currentMonth||7==this.currentMonth||8==this.currentMonth||10==this.currentMonth||12==this.currentMonth){this.newDay=new Date(this.currentYear+"/"+(this.currentMonth+1)+"/"+1)}this.currentDate=1;this.currentMonth=this.currentMonth+1}}var dynamicEl=document.createElement("lrn-calendar-date");dynamicEl.valid=!0;dynamicEl.setAttribute("id","date");dynamicEl.date=this.newDay;dynamicEl.style.width="14.25%";dynamicEl.style.display="inline-block";dynamicEl.view=this.view;if(this.date.getFullYear()===this.newDay.getFullYear()&&this.date.getMonth()===this.newDay.getMonth()&&this.date.getDate()===this.newDay.getDate()){dynamicEl.loadeddate=!0}var eventsOnDayMain=this.eventCheck(this.listOfEvents,this.newDay),sendEventMain=this.createReturn(eventsOnDayMain);dynamicEl.events=sendEventMain;if(7>firstWeekCount){dynamicEl.firstWeek=!0}firstWeekCount=firstWeekCount+1;dynamicEl.valid=!0;dom(this.calendarView).appendChild(dynamicEl);days=days+1;if(days==this.totalDays&&6!=this.newDay.getDay()){days=days-1;pastDate=1}}},/*
   ...
   */createRepeatedEvent:function(events,maxRepeat){var iter=events.iterator(events.startDate);let next,num=0;while((next=iter.next())&&num++<maxRepeat){var newTest=new ICAL.Event;newTest.summary=events.summary;newTest.description=events.description;newTest.startDate=events.getOccurrenceDetails(next).startDate;newTest.endDate=events.getOccurrenceDetails(next).endDate;var locationString=events.location+"";newTest.location=locationString;if(newTest.startDate._time.day!=events.startDate._time.day||newTest.startDate._time.month!=events.startDate._time.month||newTest.startDate._time.year!=events.startDate._time.year){this.eventArray.push(newTest)}}return},/*
   * Called from eventCheck, gets the start time of an event and returns it
   */createDate:function(event){var year=event.startDate._time.year,month=event.startDate._time.month,day=event.startDate._time.day,eventDate=new Date(year+"/"+month+"/"+day);return eventDate},/*
   ...
   */eventCheck:function(events,date){for(var eventsOnDay=[],end=events.length,i=this.startIndex,startDay;i<end;i++){startDay=this.createDate(events[i]);if(startDay.getTime()>date.getTime()){end=i;if(0!=i){this.startIndex=i-1}}if(startDay.getTime()==date.getTime()){eventsOnDay.push(events[i])}}return eventsOnDay},/*
   * Organizes the events on a certain day in order based on start time
   */sortByTime:function(eventList){var swapped;do{swapped=!1;for(var i=0;i<eventList.length-1;i++){if(eventList[i].startDate>eventList[i+1].startDate){var temp=eventList[i];eventList[i]=eventList[i+1];eventList[i+1]=temp;swapped=!0}}}while(swapped);return eventList},getDateInfo:function(){this.startMonth=this.date.getMonth()+1;this.startYear=this.date.getFullYear();this.startDay=this.date.getDate();this.startDayOfWeek=this.date.getDay();this.currentDate=this.startDay;this.currentMonth=this.startMonth;this.currentYear=this.startYear;this.currentDayofWeek=this.startDayOfWeek;return},/*
   * Creates an event array that gets all of the events for that certain day
   */createReturn:function(event){for(var EventArray=[],i=0;i<event.length;i++){EventArray.push({event:event[i]})}return EventArray},/*
   * Gets the date to display on the header, currently the month and year. Returns it as a String
   */getDisplayDate:function(date){if("function"===typeof date.getMonth){var monthInt=date.getMonth(),day=date.getDate(),monthsArray=["January","February","March","April","May","June","July","August","September","October","November","December"];return monthsArray[monthInt]+" "+date.getFullYear()}return""}});export{LrnCalendar};