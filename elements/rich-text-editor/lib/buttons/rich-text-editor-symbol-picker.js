...
class RichTextEditorSymbolPicker extends RichTextEditorPickerBehaviors(
   ...
   */
  _setOptions() {
    console.log('_setOptions',this.symbolTypes);
    this.symbols = {
      symbols: [
    ...
    this.symbolTypes.forEach(type => this.symbols[type].forEach(symbol => optData.push(symbol)));
    optData = this._setPickerOptions(optData);
    this.options = optData;
    console.log('_setOptions 2',this.symbolTypes,optData,this.options);
  }
}
