...
class LoremData extends LitElement {
          ...
          users: {
            ...
            children: {
              ...
              schema: {
                ...
                hex: { type: "hex"},
                undergrad: { type: "bool"},
                campus: { type: "option", options: ["WC","Altoona","Bubois"]},
                instructor: { 
                  type: "option",
                   weightedOptions: [
                    {value: true, weight: 1},
  ...
  randomPlaceImg(aspect, filter, topic = "any"){
    ...
    return `//placeimg.com/${aspect}/${topic}${filter === "greyscale" 
      ? "/greyscale" 
      : filter === "sepia"
      ? "/sepia"
      : ""}`;
  ...
  randomFlickr(aspect, searchTerms = [], searchAll = false, multiple = -1){
    aspect = aspect || this.randomAspect();
    return `https://loremflickr.com/${aspect}${searchTerms.length === 0 
      ? `` 
  ...
  randomImage(aspect,greyscale,topic,multiple) {
    return typeof multiple === typeof undefined 
  ...
  randomParagraph(min = 3, max = 7, sentenceMin, sentenceMax) {
    let paragraph = [];
    for(let i = this.randomNumber(min, max); i > 0; i--){
      paragraph.push(this.randomSentence(sentenceMin, sentenceMax));
    }
  ...
  randomType(schema){
    ...
    if(schema.type) switch(schema.type){
      ...
      case 'option': 
        ...
        break;
      case 'paragraph':
        val = this.randomParagraph(schema.min, schema.max, schema.sentenceMin, schema.sentenceMax);
        break;
      case 'sentence':
