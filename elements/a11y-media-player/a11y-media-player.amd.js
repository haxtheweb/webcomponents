 ...
 *
 */var A11yMediaPlayer=/*#__PURE__*/function(_A11yMediaBehaviors){babelHelpers.inherits(A11yMediaPlayer,_A11yMediaBehaviors);function A11yMediaPlayer(){babelHelpers.classCallCheck(this,A11yMediaPlayer);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(A11yMediaPlayer).apply(this,arguments))}babelHelpers.createClass(A11yMediaPlayer,[{key:"connectedCallback",/**
   * life cycle, element is afixed to the DOM
   */value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(A11yMediaPlayer.prototype),"connectedCallback",this).call(this);var root=this,name="screenfullLib",basePath=(0,_resolveUrl.pathFromUrl)(decodeURIComponent(meta.url)),location="".concat(basePath,"lib/screenfull/dist/screenfull.js");window.ESGlobalBridge.requestAvailability();window.ESGlobalBridge.instance.load(name,location);window.addEventListener("es-bridge-".concat(name,"-loaded"),root._onScreenfullLoaded.bind(root));root.__playerAttached=!0;window.A11yMediaStateManager.requestAvailability();root._addResponsiveUtility();window.dispatchEvent(new CustomEvent("a11y-player",{detail:root}));if(root.isYoutube){root._youTubeRequest()}}/**
   * sets initial values and loads video or youtube iframe
   */},{key:"ready",value:function ready(){babelHelpers.get(babelHelpers.getPrototypeOf(A11yMediaPlayer.prototype),"ready",this).call(this);var root=this,aspect=16/9,tracks=[],tdata=[],selected=0;if("object"===("undefined"===typeof screenfull?"undefined":babelHelpers.typeof(screenfull)))root._onScreenfullLoaded.bind(root);if(null===root.id)root.id="a11y-media-player"+Date.now();root.__playerReady=!0;root.target=root.shadowRoot.querySelector("#transcript");root.__status=root._getLocal("loading","label");root.__slider=root.$.slider;root.__volume=root.muted?0:Math.max(this.volume,10);root.__resumePlaying=!1;root.__duration=0;root.$.controls.setStatus(root.__status);root.width=null!==root.width?root.width:"100%";root.style.maxWidth=null!==root.width?root.width:"100%";root._setPlayerHeight(aspect);if(root.isYoutube){root._youTubeRequest();document.addEventListener("timeupdate",function(e){if(e.detail===root.media)root._handleTimeUpdate(e)})}else{root.media=root.$.html5;root.media.media.addEventListener("timeupdate",function(e){root._handleTimeUpdate(e)});root._addSourcesAndTracks()}root.$.transcript.setMedia(root.$.innerplayer);root.$.slider.addEventListener("mousedown",function(e){root._handleSliderStart()});root.$.slider.addEventListener("mouseup",function(e){root._handleSliderStop()});root.$.slider.addEventListener("keydown",function(e){root._handleSliderStart()});root.$.slider.addEventListener("keyup",function(e){root._handleSliderStop()});root.$.slider.addEventListener("blur",function(e){root._handleSliderStop()})}/**
   * plays the media
...
get:function get(){return(0,_polymerElement.html)(_templateObject_cc37bbb06a8511e9a174a9be8a344b8a())}// properties available to the custom element for data binding
   ...
   */},{key:"tag",get:function get(){return"a11y-media-player"}//get player-specific behaviors
},{key:"behaviors",get:function get(){return[_a11yMediaBehaviors.A11yMediaBehaviors]}}]);return A11yMediaPlayer}(_a11yMediaBehaviors.A11yMediaBehaviors);/**
