 ...
 */import{html,PolymerElement}from"./node_modules/@polymer/polymer/polymer-element.js";import{A11yMediaBehaviors}from"./lib/a11y-media-behaviors.js";import{pathFromUrl}from"./node_modules/@polymer/polymer/lib/utils/resolve-url.js";import"./node_modules/@lrnwebcomponents/es-global-bridge/es-global-bridge.js";import"./node_modules/@polymer/paper-slider/paper-slider.js";import"./node_modules/@polymer/iron-icons/iron-icons.js";import"./node_modules/@polymer/iron-icons/av-icons.js";import"./lib/a11y-media-controls.js";import"./lib/a11y-media-html5.js";import"./lib/a11y-media-play-button.js";import"./lib/a11y-media-transcript.js";import"./lib/a11y-media-transcript-controls.js";import"./lib/a11y-media-state-manager.js";import"./lib/a11y-media-youtube.js";export{A11yMediaPlayer};/**
 * `a11y-media-player`
...
static get behaviors(){return[A11yMediaBehaviors]}/**
   ...
   */connectedCallback(){super.connectedCallback();let root=this;const name="screenfullLib",basePath=pathFromUrl(decodeURIComponent(import.meta.url)),location=`${basePath}lib/screenfull/dist/screenfull.js`;window.ESGlobalBridge.requestAvailability();window.ESGlobalBridge.instance.load(name,location);window.addEventListener(`es-bridge-${name}-loaded`,root._onScreenfullLoaded.bind(root));root.__playerAttached=!0;window.A11yMediaStateManager.requestAvailability();root._addResponsiveUtility();window.dispatchEvent(new CustomEvent("a11y-player",{detail:root}));if(root.isYoutube){root._youTubeRequest()}}/**
   * sets initial values and loads video or youtube iframe
   */ready(){super.ready();let root=this,aspect=16/9,tracks=[],tdata=[],selected=0;if("object"===typeof screenfull)root._onScreenfullLoaded.bind(root);if(null===root.id)root.id="a11y-media-player"+Date.now();root.__playerReady=!0;root.target=root.shadowRoot.querySelector("#transcript");root.__status=root._getLocal("loading","label");root.__slider=root.$.slider;root.__volume=root.muted?0:Math.max(this.volume,10);root.__resumePlaying=!1;root.__duration=0;root.$.controls.setStatus(root.__status);root.width=null!==root.width?root.width:"100%";root.style.maxWidth=null!==root.width?root.width:"100%";root._setPlayerHeight(aspect);if(root.isYoutube){root._youTubeRequest();document.addEventListener("timeupdate",e=>{if(e.detail===root.media)root._handleTimeUpdate(e)})}else{root.media=root.$.html5;root.media.media.addEventListener("timeupdate",e=>{root._handleTimeUpdate(e)});root._addSourcesAndTracks()}root.$.transcript.setMedia(root.$.innerplayer);root.$.slider.addEventListener("mousedown",e=>{root._handleSliderStart()});root.$.slider.addEventListener("mouseup",e=>{root._handleSliderStop()});root.$.slider.addEventListener("keydown",e=>{root._handleSliderStart()});root.$.slider.addEventListener("keyup",e=>{root._handleSliderStop()});root.$.slider.addEventListener("blur",e=>{root._handleSliderStop()})}/**
   * plays the media
