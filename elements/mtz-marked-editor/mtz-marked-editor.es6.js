...
*/let MtzMarkedEditor=Polymer({_template:html`
  ...
  `,is:"mtz-marked-editor",properties:{autofocus:Boolean,readonly:Boolean,textareaSelector:{type:String,value:"textarea"},__textarea:Object},ready(){this.__bindControlToEditor=this.__bindControlToEditor.bind(this)},attached(){this.addEventListener("register-control",this.__bindControlToEditor);this.__textarea=dom(this).queryDistributedElements("[slot=\"textarea\"]")[0]},detached(){this.removeEventListener("register-control",this.__bindControlToEditor)},/**
...
dom(event).rootTarget.__editor=this}});export{MtzMarkedEditor};