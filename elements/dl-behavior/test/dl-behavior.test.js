...
describe("MtzFileDownloadBehaviors mixin test", () => {
  ...
  beforeEach(async () => {
    element = await fixture(html`<test-download-element></test-download-element>`);
  ...
  it("downloadFromURI opens window with correct parameters", async () => {
    ...
    const result = element.downloadFromURI('https://example.com/file.pdf', true);
  ...
  it("handles special characters in filenames", async () => {
    // Setup mocks
    globalThis.Blob = function() { return { size: 0, type: 'text/plain' }; };
    globalThis.URL = globalThis.URL || {};
    globalThis.URL.createObjectURL = () => 'blob:mock-url';
