...
`,is:"sites-listing",properties:{sites:{type:Array},size:{type:String,value:"large"},dataSource:{type:String}},_siteClicked:function(e){var normalizedEvent=dom(e),local=normalizedEvent.localTarget,active=local.getAttribute("data-site-id");let findSite=this.sites.filter(site=>{if(site.id!==active){return!1}return!0});if(0<findSite.length){findSite=findSite.pop()}if(typeof findSite.location!==typeof void 0){window.location.href=findSite.location}},_mouseEnter:function(e){let card=dom(e.target).querySelectorAll("site-card")[0];card.__oldElevation=card.elevation;if(5<card.elevation+2){card.elevation=5}else{card.elevation+=2}},_mouseLeave:function(e){let card=dom(e.target).querySelectorAll("site-card")[0];card.elevation=card.__oldElevation}});