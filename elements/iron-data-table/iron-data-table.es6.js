...
*/let IronDataTable=Polymer({_template:html`
  ...
  `,is:"iron-data-table",behaviors:[IronResizableBehavior],listeners:{"column-filter-changed":"_onColumnFilterChanged","iron-resize":"_resizeCellContainers","item-changed":"_itemChanged",scroll:"_onHorizontalScroll"},properties:{/**
     ...
     */filter:{type:Array,notify:!0,value:function(){return[]}},/**
     ...
     */selectedItems:{type:Object,notify:!0,readOnly:!0,value:function(){var items=[];items.filters=[];return items}},/**
     ...
     */sortOrder:{type:Array,notify:!0,value:function(){return[]}},/**
     * An array of `data-table-column` elements which contain the templates
     * to be stamped with items.
     */columns:{type:Array,notify:!0,value:function(){return[]},observer:"_columnsChanged"},/**
...
this.clearSelection();this.clearCache();this.$.list.scrollToIndex(0)},_sortDirectionChanged:function(e){for(var i=0;i<this.sortOrder.length;i++){if(this.sortOrder[i].path===e.detail.path){if(e.detail.direction){this.set("sortOrder."+i+".direction",e.detail.direction)}else{this.splice("sortOrder",i,1)}return}}this.push("sortOrder",{path:e.detail.path,direction:e.detail.direction})},_columnsChanged:function(columns,oldColumns){if(oldColumns){oldColumns.forEach(function(column){this.unlisten(column,"filter-value-changed")}.bind(this))}if(columns){columns.forEach(function(column){column.table=this;this.listen(column,"filter-value-changed","_onColumnFilterChanged")}.bind(this))}},_onColumnFilterChanged:function(e){for(var i=0;i<this.filter.length;i++){if(this.filter[i].path===e.detail.filterBy){this.set("filter."+i+".filter",e.detail.value);// selectedItems.filter is actually already set at this point when
// clearSelection is called after filter is set.
this.set("selectedItems.filters."+i+".filter",e.detail.value);return}}this.push("filter",{path:e.detail.filterBy,filter:e.detail.value});this.push("selectedItems.filters",{path:e.detail.filterBy,filter:e.detail.value})},_resizeCellContainers:function(){// reset header width first to make the cells and scroll width to reset their widths.
this.$.container.style.width="";async.microTask.run(()=>{this.$.container.style.width=Math.min(this.scrollWidth,this.clientWidth+this.scrollLeft)+"px";// add scrollbar width as padding
...
},1e3)}},_onVerticalScroll:function(e){// Toggle shadow when at the top
...
this.$.list._collection.store[index]=item;if(item&&"object"==typeof item){this.$.list._collection.omap.set(item,index)}else{this.$.list._collection.pmap[item]=index}}// resize required for variable row height items.
...
if(size>oldSize){var oldLastPage=Math.floor(oldSize/this.pageSize);if(this._isPageCached(oldLastPage)||0===oldLastPage){this.refreshPage(oldLastPage)}}},/**
   ...
   */clearCache:function(){this._cachedPages=[];// Force reload on currently visible pages.
this.refreshPage(this._currentPage)},/**
   * Clears the cache for a page and reloads the data from datasource.
   */refreshPage:function(page){if(this._cachedPages){var index=this._cachedPages.indexOf(page);if(-1<index){this.splice("_cachedPages",index,1)}}this._currentPageChanged(this.dataSource,page)},_updateSizeForItem:function(event){if(event.model.get("item")){// notifyResize() doesn't do anything on iOS if the viewport size hasn't changed
...
this.$.list._updateMetrics(itemSet);this.$.list._positionItems()}},/**
   ...
   */expandItem:function(item){if(this.rowDetail&&this._expandedItems&&!this._isExpanded(item,this._expandedItems)){// replacing the whole array here to simplify the observers.
this._expandedItems.push(item);this._expandedItems=this._expandedItems.slice(0)}},/**
   ...
   */collapseItem:function(item){if(this.rowDetail&&this._expandedItems&&this._isExpanded(item,this._expandedItems)){var index=this._expandedItems.indexOf(item);// replacing the whole array here to simplify the obsevers.
this._expandedItems.splice(index,1);this._expandedItems=this._expandedItems.slice(0)}},_isExpanded:function(item,items){return items&&-1<items.indexOf(item)},_isFocusable:function(target){if((void 0).useNativeShadow){// https://nemisj.com/focusable/
...
// the shadow root.
return target.contains(dom(document.activeElement).node)||"A"===target.tagName.toUpperCase()}},/**
...
_onCellClick:function(e){// Prevent item selection if row itself is not focused. This means that
