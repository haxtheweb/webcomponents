 ...
 */let LrndesignImagemap=Polymer({_template:html`
    <style>
      :host {
        display: block;
      }
    ...
    ></iron-ajax>
  `,is:"lrndesign-imagemap",properties:{/**
   ...
   * attached life cycle
   */attached:function(){window.SimpleModal.requestAvailability();window.addEventListener("simple-modal-closed",e=>{if(e.detail.invokedBy===this){this.closeHotspot()}})},/**
   * detached life cycle
   */detached:function(){window.removeEventListener("simple-modal-closed",e=>{if(e.detail.invokedBy===this){this.closeHotspot()}})},/**
   * Convert from svg text to an array in the table function
   */_getSVGHandler:function(e){let root=this,temp=document.createElement("div"),getID=function(element,alt){if(null===element.getAttribute("id"))element.setAttribute("id",alt);return element.getAttribute("id")},setAriaLabelledBy=function(source,target,prefix){// adds title and desc elements to target and sets the aria-labelledby attribute
...
temp.innerHTML=e.detail.response;let svg=temp.querySelector("svg"),svgid=getID(svg,"svg-"+Date.now()),hdata=dom(root).querySelectorAll("lrndesign-imagemap-hotspot");setAriaLabelledBy(root,svg,svgid);this.$.svg.appendChild(svg);for(let i=0;i<hdata.length;i++){let hid=hdata[i].getAttribute("hotspot-id"),hotspot=svg.querySelector("#"+hid),clone=svg.cloneNode(!0);//clone svg for print versions and show hotspot as selected
setAriaLabelledBy(hdata[i],clone,hid);hdata[i].appendChild(clone);hdata[i].querySelector("#"+hid).classList.add("selected");hdata[i].setParentHeading(root.$.heading);for(let j=0;j<hdata.length;j++){hdata[i].querySelector("#"+hdata[j].getAttribute("hotspot-id")).classList.add("hotspot")}//configure hotspot on main (interactive) svg
let hbutton=document.createElement("button");hbutton.setAttribute("tabindex",0);hbutton.setAttribute("aria-label",hdata[i].label);root.$.buttons.appendChild(hbutton);hbutton.addEventListener("focus",function(){console.log("focus",i,hotspot);hotspot.classList.add("focus")});hbutton.addEventListener("blur",function(){hotspot.classList.remove("focus")});hotspot.classList.add("hotspot");hotspot.addEventListener("click",e=>{this.openHotspot(hotspot,hdata[i])});hbutton.addEventListener("keyup",e=>{if(13===e.keyCode||32===e.keyCode){if(!hotspot.classList.contains("selected")){this.openHotspot(hotspot,hdata[i])}}})}},/**
   ...
   */openHotspot:function(hotspot,details){// get everything flat
var children=details.$.desc.querySelector("slot").assignedNodes({flatten:!0});let c=document.createElement("div");// append clones of the children we found
for(var child in children){c.appendChild(children[child].cloneNode(!0))}const evt=new CustomEvent("simple-modal-show",{bubbles:!0,cancelable:!0,detail:{title:details.getAttribute("label"),elements:{content:c},invokedBy:this,clone:!1}});this.dispatchEvent(evt);this.__activeHotspot=hotspot;this.resetHotspots();hotspot.classList.add("selected")},/**
   * Closes a hotspot.
   */closeHotspot:function(){this.resetHotspots();this.__activeHotspot.focus()},/**
   * Closes dialog and deselects all hotspots.
   */resetHotspots:function(){let hotspots=this.querySelectorAll(".hotspot[role=\"button\"]");for(let i=0;i<hotspots.length;i++){hotspots[i].classList.remove("selected")}}});export{LrndesignImagemap};