...
class HaxStore extends I18NMixin(winEventsElement(HAXElement(LitElement))) {
  ...
  constructor() {
    ...
    // Human-readable names for the named HTML entities used by the symbol
    // picker. SimplePickerSymbols only carries the entity + alt (the glyph),
    // so without this map every row reads "symbols: ©". Entities not listed
    // here fall back to a title-cased version of the entity name.
    const SYMBOL_NAMES = {
      ...
      "&prime;": "Prime",
      "&Prime;": "Double prime",
      "&lsaquo;": "Single left guillemet",
      "&rsaquo;": "Single right guillemet",
      "&oline;": "Overline",
      "&euro;": "Euro sign",
      "&trade;": "Trademark",
      "&larr;": "Left arrow",
    ...
    SuperDaemonInstance.defineOption({
      ...
      value: {
        ...
        program: async (input, values) => {
          ...
          // title-cased version of the entity name when unmapped
          const nameFor = (entity) => {
            if (SYMBOL_NAMES[entity]) {
              return SYMBOL_NAMES[entity];
            }
            const raw = entity.replace(/^&/, "").replace(/;$/, "");
  ...
  async _insertTextResult(text) {
    ...
    if (
      ...
      // Mark as inserted so close() does NOT restore the stripped token
      // (the emoji/symbol replaced it). Without this, close() would re-insert
      // ":: " on top of the just-inserted emoji.
      SuperDaemonInstance.inlineTextInsert = false;
      return;
    }
