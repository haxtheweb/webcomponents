...
class LecturePlayer extends DDDSuper(LitElement) {
  ...
  addPrevNextListeners(){
    ...
    nextSlideBtn.addEventListener('click', () => {
      const anchorsWithId = document.querySelectorAll('lecture-anchor[id]').length;
  ...
  updatePlaylist(){
    ...
    document.querySelector('.timestampList').scrollTo({ left: document.querySelector('.timestampBtn.active').offsetLeft - 125, behavior: 'smooth' });
  }
  ...
  seek(timestamp){
    ...
    if(document.querySelector('video-player').shadowRoot.querySelector("a11y-media-player")){
      document.querySelector('video-player').shadowRoot.querySelector("a11y-media-player").seek(timestamp);
    }
  }

  play(){
    if(document.querySelector('video-player').shadowRoot.querySelector("a11y-media-player")){
      document.querySelector('video-player').shadowRoot.querySelector("a11y-media-player").play();
    }
  }
  ...
  endVideo(){
    ...
    document.querySelector('video-player').shadowRoot.querySelector("a11y-media-player").pause();
