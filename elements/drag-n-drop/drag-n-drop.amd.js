...
get:function get(){return"\n<style>:host {\n  display: block;\n}\n\n:host([hidden]) {\n  display: none;\n}\n</style>\n<slot></slot>"}// properties available to the custom element for data binding
   ...
   * life cycle
   */}]);function DragNDrop(){var _this,delayRender=0<arguments.length&&arguments[0]!==void 0?arguments[0]:!1;babelHelpers.classCallCheck(this,DragNDrop);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(DragNDrop).call(this));// set tag for later use
...
var obj=DragNDrop.properties;for(var p in obj){if(obj.hasOwnProperty(p)){if(_this.hasAttribute(p)){_this[p]=_this.getAttribute(p)}else{_this.setAttribute(p,obj[p].value);_this[p]=obj[p].value}}}// optional queue for future use
...
}]);return DragNDrop}(babelHelpers.wrapNativeSuper(HTMLElement));_exports.DragNDrop=DragNDrop;window.customElements.define(DragNDrop.tag,DragNDrop)});