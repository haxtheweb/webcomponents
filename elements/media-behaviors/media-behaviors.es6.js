 ...
 **/window.MediaBehaviors.Video={/**
   ...
   */_sourceIsIframe:function(source){let type=this.getVideoType(source);if("local"==type){return!1}else{return!0}},/**
   * Check source of the video, potentially correcting bad links.
   */cleanVideoSource:function(input,type){if("local"!=type){// strip off the ? modifier for youtube/vimeo so we can build ourselves
...
if(-1!=input.indexOf("/videos/")){input=input.replace("/videos/","/")}return input.replace("vimeo.com/","player.vimeo.com/video/")}// copy and paste from the URL
   ...
   */getVideoType:function(source){let localFormats=["aac","flac","mov","mp3","mp4","oga","ogg","ogv","wav","webm"],isLocal=!1;// some common ones
