...
window.MediaBehaviors.Video = {
  ...
  cleanVideoSource: function cleanVideoSource(input, type) {
    if ("local" != type) {
      ...
      if (
        -1 == input.indexOf("player.vimeo.com") &&
        -1 != input.indexOf("vimeo.com")
      ) {
        if (-1 != input.indexOf("/videos/")) {
          input = input.replace("/videos/", "/");
        }
        return input.replace("vimeo.com/", "player.vimeo.com/video/");
      } else if (-1 != input.indexOf("youtube.com/watch")) {
        return input.replace("youtube.com/watch", "youtube.com/embed/") + v;
      } else if (-1 != input.indexOf("youtube-no-cookie.com/embed")) {
        return (
          input.replace("youtube-no-cookie.com/embed", "youtube.com/embed/") + v
        );
      } else if (-1 != input.indexOf("youtu.be")) {
        return input.replace("youtu.be/", "www.youtube.com/embed/") + v;
      } else if (
        -1 != input.indexOf("sketchfab.com") &&
        -1 == input.indexOf("/embed")
      ) {
        return input + "/embed";
      } else if (
        -1 != input.indexOf("dailymotion.com") &&
        -1 == input.indexOf("/embed")
      ) {
  ...
  getVideoType: function getVideoType(source) {
    var localFormats = [
      ...
      isLocal = !1;
    if (-1 != source.indexOf("vimeo")) {
      return "vimeo";
    } else if (
      -1 != source.indexOf("youtube") ||
      -1 != source.indexOf("youtu.be")
    ) {
      return "youtube";
    } else if (-1 != source.indexOf("sketchfab.com")) {
      return "sketchfab";
    } else if (-1 != source.indexOf("dailymotion.com")) {
      return "dailymotion";
    }
    for (var i = 0; i < localFormats.length; i++) {
      if (!isLocal && -1 < source.toLowerCase().indexOf("." + localFormats[i]))
        isLocal = !0;
    }
    if (isLocal) {
      return "local";
