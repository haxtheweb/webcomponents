...
*/let GrafittoFilter=Polymer({_template:html`
    <div id="dom"><slot></slot></div>
  `,is:"grafitto-filter",behaviors:[Templatizer,OptionalMutableDataBehavior],properties:{/**
   ...
   */filter:function(){//This forces _computeFiltered function to do its job :-)
this.where=""},/**
   ...
   */_computeFiltered:function(items,where,like,caseSensitive){var regex=null;if(caseSensitive){regex=new RegExp(like)}else{regex=new RegExp(like,"i")}var filtered=[];if(this.f){var customFunction=this.f.bind(this);filtered=items.filter(customFunction)}else{//Save a reference to this object
...
if("number"==typeof item){return regex.test(item.toString())}})}return filtered},/**
   ...
   */_populateUserTemplate:function(filtered){// set after template is stamped
...
dom(this).appendChild(this.__clone.root)},/**
   ...
   */_decomposeWhere:function(where,item){return where.split(".").reduce(function(a,b){return a&&a[b]},item)},/**
   * The `filter` event is fired whenever filtering is done before populating the dom.
   *
   * @event filter
   */_onFilter:function(){this.fire("filter")}});export{GrafittoFilter};