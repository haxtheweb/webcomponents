...
export class MarkTheWords extends I18NMixin(LitElement) {
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (propName === "numberCorrect" && this.numberCorrect === this.numberGuessed && this.numberCorrect !== 0 && this.numberGuessed !== 0) {
  ...
  checkAnswer(e) {
    ...
    if(this.isEnabled){
      ...
      const selected = this.shadowRoot.querySelectorAll(".text span[data-selected]");
  ...
  render() {
    ...
    <confetti-container id="confetti">
      ...
      ${this._haxstate && this.demoMode ? html`<slot></slot>` : html`<div class="text"></div>`}      
      <div class="buttons">
        <simple-toolbar-button @click="${this.checkAnswer}" icon="check" label="${this.isEnabled ? this.t.check : this.t.tryAgain}" show-text-label icon-position="left" align-vertical="center" align-horizontal="center"></simple-toolbar-button>
        ${this.isEnabled ? `` :
        html`${this.numberGuessed}/${this.numberCorrect} ${Math.round(10*((this.numberGuessed/this.numberCorrect) * 100))/10}%`}
        ${this._haxstate ? html`<simple-toolbar-button @click="${this.toggleDemo}" icon="${this.demoMode ? "av:play-circle-filled" : "image:edit"}" label="${this.demoMode ? this.t.tryMode : this.t.editMode}" show-text-label icon-position="left" align-vertical="center" align-horizontal="center"></simple-toolbar-button>` : ``}
      </div>
    </confetti-container>
    `;
  }
