define([
  ...
  function _templateObject_801d2900f32e11e8beed89607d3c8e9d() {
    ...
    _templateObject_801d2900f32e11e8beed89607d3c8e9d = function _templateObject_801d2900f32e11e8beed89607d3c8e9d() {
      return data;
    };
    return data;
  }
  var ProgressDonut = (0, _polymerLegacy.Polymer)({
    _template: (0, _polymerLegacy.html)(
      _templateObject_801d2900f32e11e8beed89607d3c8e9d()
    ),
    is: "progress-donut",
    behaviors: [
    ...
    listeners: { "chartist-render-draw": "_onCreated" },
    properties: {
    ...
    _getImageStyle: function _getImageStyle(size) {
      ...
      if ("xs" === this.size) {
        offset = "32%";
        diameter = "36%";
      } else if ("sm" === this.size) {
        offset = "26%";
        diameter = "48%";
      } else if ("lg" === this.size) {
        offset = "20%";
        diameter = "60%";
      } else if ("xl" === this.size) {
        offset = "17%";
        diameter = "66%";
      }
      return (
    ...
    _onCreated: function _onCreated(e) {
      this.__chart = e.detail;
      this.makeChart(this.__chart);
    },
    makeChart: function makeChart(chart) {
      if (chart !== void 0) {
        var colors = this.colors,
          strokeWidth = "10%",
          hex = window.SimpleColorsUtility.hexCodes,
          accent =
            null !== this.accentColor
              ? this.accentColor.replace(/-([a-z])/g, function(g) {
                  return g[1].toUpperCase();
                })
              : null;
        if (colors === void 0 || null === colors || 0 === colors.length) {
          if (null !== accent && null !== hex[accent]) {
            colors = this.dark
              ? [
                  hex[accent][9],
                  hex[accent][6],
