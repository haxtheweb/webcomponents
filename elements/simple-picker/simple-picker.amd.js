...
[
   ...
   */__selectedOption:{type:Object}};if(babelHelpers.get(babelHelpers.getPrototypeOf(SimplePicker),"properties",this)){props=Object.assign(props,babelHelpers.get(babelHelpers.getPrototypeOf(SimplePicker),"properties",this))}return props}/**
...
var obj=SimplePicker.properties;for(var p in obj){if(obj.hasOwnProperty(p)){if(_this.hasAttribute(p)){_this[p]=_this.getAttribute(p)}else{if(p.reflect)_this.setAttribute(p,obj[p].value);_this[p]=obj[p].value}}}return _this}/**
   * life cycle, element is afixed to the DOM
   */babelHelpers.createClass(SimplePicker,[{key:"connectedCallback",value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(SimplePicker.prototype),"connectedCallback",this).call(this)}// static get observedAttributes() {
//   return [];
// }
// disconnectedCallback() {}
/*attributeChangedCallback(name, oldval, newval) {
    super.attributeChangedCallback(name, oldval, newval);
  }*/},{key:"updated",value:function updated(changedProperties){var _this3=this;changedProperties.forEach(function(oldValue,propName){if("value"===propName)_this3._valueChanged(_this3.value,oldValue);if("options"===propName)_this3._optionsChanged(_this3.value,oldValue)});this.dispatchEvent(new CustomEvent("changed",{detail:this}))}/**
   ...
   * @returns {void}
   */},{key:"_goToOption",value:function _goToOption(rownum,colnum){var targetId=(0,_litElement.html)(_templateObject5_b7053a60fffe11e9abf7bfc32b7d31e7(),rownum,colnum),target=this.shadowRoot.querySelector("#"+targetId),active=this.shadowRoot.querySelector("#"+this.__activeDesc);if(null!==target){target.tabindex=0;//allow item to be focusable.
target.focus();active.tabindex=-1;//prevent tabbing between options.
   ...
   * @returns {void}
   */},{key:"_handleListboxClick",value:function _handleListboxClick(e){this.dispatchEvent(new CustomEvent("click",{detail:this}));this._toggleListbox()}/**
   * handles listbox click event
   *
   * @param {event} e event
   * @param {string} type type of event
   * @returns {void}
   */},{key:"_handleListboxMousedown",value:function _handleListboxMousedown(e){this.dispatchEvent(new CustomEvent("mousedown",{detail:this}))}/**
...
this._goToOption(rownum+1,[0])}}}}/**
   ...
   * @returns {void}
   */},{key:"_valueChanged",value:function _valueChanged(newValue,oldValue){this._setSelectedOption(newValue,oldValue);this.dispatchEvent(new CustomEvent("value-changed",{detail:this}))}/**
...
this.__activeDesc="option-".concat(i,"-").concat(j);sel=this.__options[i][j]}}}}if(null===sel)this.value=null;this.__selectedOption=sel;this.dispatchEvent(new CustomEvent("change",{bubbles:!0,detail:this}))}/**
   * toggles listbox
   *
   * @param {boolean} open whether to open
   * @returns {void}
   */},{key:"_toggleListbox",value:function _toggleListbox(){var open=0<arguments.length&&arguments[0]!==void 0?arguments[0]:!this.expanded;if(this.disabled)return;var active=this.shadowRoot.querySelector("#"+this.__activeDesc);this.expanded=open;if(open){if(null!==active)active.focus();this.dispatchEvent(new CustomEvent("expand",{detail:this}))}else{if(null!==active)this.value=active.getAttribute("value");this.dispatchEvent(new CustomEvent("collapse",{detail:this}))}}/**
