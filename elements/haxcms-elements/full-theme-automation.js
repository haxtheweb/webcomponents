...
async function runFullAutomation() {
  ...
  for (let i = 0; i < themes.length; i++) {
    ...
    const switchCode = ThemeScreenshotAutomation.generateThemeSwitchCode(themeElement);
    ...
    console.log(`1. puppeteer_evaluate: { "script": "HAXCMS.setTheme('${themeElement}')" }`);
    console.log(`2. Wait ${config.delay}ms`);
    console.log(`3. puppeteer_screenshot: { "name": "${themeElement}", "width": ${config.width}, "height": ${config.height} }`);
  ...
  console.log(`The AI agent should execute the MCP tool calls for each theme in sequence.`);
}
