...
export class SiteSkeletonGenerator {
  ...
  static getSiteMetaName(manifest) {
    if (manifest.metadata && manifest.metadata.site && manifest.metadata.site.name) {
  ...
  static extractCustomThemeProperties(theme) {
    const custom = {};
    const standardProps = ['element', 'variables', 'hexCode', 'cssVariable', 'icon', 'image'];
  ...
  static extractRelevantMetadata(metadata) {
    const relevant = {};
    const preserveFields = ['wordCount', 'difficulty', 'audience', 'contentType'];
  ...
  static toAppHaxFormat(skeleton, customizations = {}) {
    ...
    const apiData = {
      site: {
        ...
        description: customizations.siteDescription || skeleton.site.description,
