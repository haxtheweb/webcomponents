 ...
 */
class SiteOutlineBlock extends LitElement {
  /**
  ...
  render() {
    return html`
      <div class="wrapper">
        ...
        <site-query
          @result-changed="${this._onResultChanged}"
          sort='{"order": "ASC"}'
            ...
            <div class="spacing">
              <a
                ...
                ><button ?noink="${this.noink}">
                  <span class="link-index">${this._humanIndex(index)}</span
  ...
  _activeIdChanged(newValue) {
    ...
    if (this.indicator != "none") {
      if (newValue) {
        ...
        } else if (this.manifest && this.manifest.items) {
