...
*/let PaperIconPicker=Polymer({_template:html`
   ...
   */properties:{/**
     ...
     * icons.
     */iconList:{type:Array,notify:!0,value:function(){return[]}},/**
     ...
     */noink:{type:Boolean}},/**
   * on open event
   */_onOpen:function(e){setTimeout(()=>{try{this.shadowRoot.querySelector("paper-item").focus()}catch(error){}},500)},/**
   * Close list.
   */close:function(){this.opened=!1},/**
   * List that we will output to the screen
   */_computeRenderIconList:function(list){var renderList=[],item={};for(var i in list){item={};if(typeof list[i].icon===typeof void 0){item.icon=list[i]}else{item.icon=list[i].icon}if(typeof list[i].index===typeof void 0){item.index=0}else{item.index=list[i].index}renderList.push(item)}return renderList},/**
   * Icon text from icon selected
   */_computedIconText:function(icon){if(""==icon){return"Select an icon"}return icon},created:function(){// Note: we won't actually render these icon boxes unless the menu is
// actually tapped.
this._renderedIcons=!1},/**
   * Attached life cycle
   */attached:function(){// need to access iconset imperatively now
const iconSets=new IronMeta({type:"iconset"});if(0===this.iconList.length&&typeof iconSets!==typeof void 0&&iconSets.list&&iconSets.list.length){var iconList=[],index=0;iconSets.list.forEach(function(item){index++;item.getIconNames().forEach(icon=>{iconList.push({icon:icon,index:index})})});this.set("iconList",iconList)}// Fit the icon boxes in columns. We first need to get the width of
