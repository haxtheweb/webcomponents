 ...
 *   // @see @haxtheweb/slide-deck/slide-deck.js
 *   processFileUpload : "haxprocessFileUpload", // input (values, response, store) invoked from the generic tray upload / drag-drop path (HAXStore.applyFileUploadTransform) AFTER a file is uploaded but BEFORE the element is inserted. values is {source, title} of the uploaded file (source is the raw uploaded path), response is the parsed upload response (response.data.file.uuid is present), store is the HAXStore instance. Return false to decline (the file is inserted normally with its raw source). Return { fileUuid, operation, valueMapping, fallbackType? } to run a @site/updateFileByUuid post-upload transform: the store calls the op, maps the result via the dot-path valueMapping (e.g. "data.deckPath"), and sets the inserted element source to the mapped path. fallbackType (optional) sets the gizmo type used for insertion if the transform fails, so the file degrades to e.g. a plain link instead of a broken specialized element. This is the generic-tray/drag-drop equivalent of the per-field schema uploadTransform; it lets an element own its post-upload conversion without the platform hard-coding its file type.
 *
