import{html,Polymer}from"./node_modules/@polymer/polymer/polymer-legacy.js";import"./lib/gitgraph.js/src/gitgraph.js";/**
`lrn-gitgraph`
A LRN element
...
*/let LrnGitgraph=Polymer({_template:html`
    <style>
      :host {
        display: block;
        overflow-x: scroll;
  ...
  `,is:"lrn-gitgraph",properties:{commits:{type:Array,value:[]},template:{type:String,value:"blackarrow"},orientation:{type:String,value:"horizontal"},mode:{type:String,value:""},reverseArrow:{type:Boolean,value:!1},/**
...
branches[item.branch].commit({sha1:item.commit,message:item.subject,author:item.author,tag:item.refs})})}}},_treeRemoveDuplicates:function(tree){var htTree=[],htCommits=[];tree.forEach(function(t){if(!htCommits.includes(t.commit)){htTree.push(t);htCommits.push(t.commit)}});return htTree},ready:function(){var root=this,config={template:root.template,// could be: "blackarrow" or "metro" or `myTemplate` (custom Template object)
reverseArrow:!1,// to make arrows point to ancestors, if displayed
orientation:root.orientation,element:root.shadowRoot.querySelector("#gitGraph")};if(""!==root.mode){config.mode=root.mode}root.config=config}});export{LrnGitgraph};