 ...
 *  -
 */let LrnsysOutline=Polymer({_template:html`
    <style>
      :host {
        display: block;
      }
  ...
  `,is:"lrnsys-outline",listeners:{"delete-item":"_handleRemoveItem","indent-item":"_handleIndentItem","focus-item":"_handleFocusItem","add-item":"_handleAddItem","move-item":"_handleMoveItem","change-item":"_handleChangeItem","focus-item":"_handleFocusItem","blur-item":"_handleBlurItem"},properties:{data:{type:Array,value:null},items:{type:Array,value:null,notify:!0},activeItem:{type:Object,notify:!0}},/**
   * Display directions for keyboard usage
   */openDirections:function(e){this.$.modal.opened=!0},/**
   * Display directions for keyboard usage
   */closeDirections:function(e){this.$.modal.opened=!1;async.microTask.run(()=>{setTimeout(()=>{this.$.dialogtrigger.focus()},50)})},/**
   * Attached lifecycle
   */attached:function(){window.SimpleModal.requestAvailability();this.__modal=this.$.modal;document.body.addEventListener("iron-overlay-canceled",this._accessibleFocus.bind(this));// fix stack order
document.body.appendChild(this.$.modal)},/**
   * Set ourselves as having focus after the modal closes.
   */_accessibleFocus:function(e){// this is OUR modal, we found her, oh modal, We've missed
...
async.microTask.run(()=>{setTimeout(()=>{this.$.dialogtrigger.focus()},50)})}},ready:function(){if(null===this.data||1>this.data.length){this.__tempid=this.__tempid===void 0?0:this.__tempid+1;this.data=[{id:"outline-item-"+this.__tempid,title:"",order:0,parent:null}]}this.setData(this.data)},/**
   ...
   */moveItem:function(item,moveUp){let sourceStart=item.index,sourceEnd=this._getLastChild(item),sourceCount=sourceEnd-sourceStart+1,target=moveUp?this.items[sourceStart].prevSibling:this._getLastChild(this.items[sourceEnd+1])-sourceCount+1;if(-1<target&&target<this.items.length){if(moveUp&&!item.disableUp||!moveUp&&!item.disableDown){let item2=this.splice("items",sourceStart,sourceCount);this.splice("items",target,0,item2);this.__focusedItem=this.$.itemslist.querySelectorAll("lrnsys-outline-item")[target];this.setData(this.items);if(this.__focusedItem!==void 0&&null!==this.__focusedItem){async.microTask.run(()=>{setTimeout(()=>{this.__focusedItem.focus()},50)})}}}},/**
   * adjust indent
   */_adjustIndent:function(item,amount){if(0<amount&&!item.disableRight||0>amount&&!item.disableLeft){let i=parseInt(item.index),oldIndent=item.indent,indent=item.indent+amount,n=i+1,prevParent=null!==item.prevSibling&&typeof item.prevSibling!==typeof void 0?item.prevSibling.id:null,grandParent=this._getItemById(item.parent)&&this._getItemById(item.parent).parent?this._getItemById(item.parent).parent.id:null;item.indent=indent;item.parent=0<amount?prevParent:grandParent;item.prevSibling=this._getSibling(i,indent,!0);item.nextSibling=this._getSibling(i,indent,!1);item.disableUp=null===item.prevSibling;item.disableDown=null===item.nextSibling;item.disableLeft=0===indent;item.disableRight=null===this.items[i-1]||typeof this.items[i-1]===typeof void 0||indent>this.items[i-1].indentLevel;this.set(`items.${i}`,item);this.notifyPath(`items.${i}.*`);while(null!==this.items[n]&&this.items[n]!==void 0&&oldIndent<this.items[n].indentLevel){this.items[n].indentLevel=this.items[n].indentLevel+amount;this.notifyPath(`items.${n}.indentLevel`);n++;next=this.items[n]}}},/**
   * gets all children of an item
   */_getLastChild:function(item){let next=item!==void 0&&null!==item?this._getSibling(item.index,item.indent,!1):null;if(null!==next&&next!==void 0){return next-1}else if(typeof item!==typeof void 0&&null!==item.parent&&null!==item.parent&&null!==this._getItemById(item.parent)){return this._getLastChild(this._getItemById(item.parent))}else{return this.items.length-1}},/**
   * converts a nested array of items and returns a flat list with indents
   */_getIndent:function(data,i){if(typeof data[i].parent!==typeof void 0){let k=data.findIndex(j=>j.id===data[i].parent);if(-1!==k&&typeof data[k]!==typeof void 0&&data[k].indent!==void 0){return data[k].indent+1}}return 0},/**
   * returns order relative to siblings
   */_getOrder:function(item){let ctr=0,order=0;for(var i in this.items){if(this.items[i].parent==item.parent&&this.items[i].id==item.id){order=ctr}else if(this.items[i].parent==item.parent){ctr++}}return order},/**
   * returns previous or next sibling
   */_getSibling:function(index,indent,prev){let inc=prev?-1:1,i=index+inc,sib=null;if(null!==this.items){while(i<this.items.length&&-1<i){if(null===sib&&typeof this.items[i]!==typeof void 0&&typeof this.items[index]!==typeof void 0&&this.items[i].parent===this.items[index].parent){sib=i}i+=inc}}return sib},/**
   * get an item by item id
   */_getItemById:function(id,offset){let i=this.items.findIndex(j=>j.id===id);offset=offset===void 0?0:offset;if(this.items[i+offset]!==void 0){return this.items[i+offset]}else{return null}},/**
   * listener to add an item
   */_handleAddItem:function(e){this.addItem(e.detail)},/**
   * listener to delete an item
   */_handleRemoveItem:function(e){this.activeItem=e.detail.item;this.removeItem(e.detail.item)},/**
   * listener to move an item
   */_handleMoveItem:function(e){this.activeItem=e.detail.item;this.moveItem(e.detail.item,e.detail.moveUp,e.detail.byGroup)},/**
   * listener to move focus up or down
   */_handleFocusItem:function(e){let item=e.detail.moveUp?e.detail.item.previousElementSibling:e.detail.item.nextElementSibling;item.setSelection()},/**
   * listener to increase or decrease indent
   */_handleIndentItem:function(e){let amt=e.detail.increase?1:-1;this._adjustIndent(this._getItemById(e.detail.item.id),amt);this.setData(this.items)},/**
   * listener to handle changes to text inputs
   */_handleChangeItem:function(e){if(null!=this._getItemById(e.detail.item.id)){let i=this.items.findIndex(j=>j.id===e.detail.item.id);if(typeof this.items[i]!==typeof void 0){this.items[i].title=e.detail.value;this.notifyPath(`items.${i}.title`)}}},/**
   * listener for focus or mouseover
   */_handleFocusItem:function(e){this.__focusedItem=e.srcElement},/**
   * listener for blur or mouseout
   */_handleBlurItem:function(e){}});export{LrnsysOutline};