...
if (!prevVersion || !nextVersion) {
  console.error(
    "Error: you must pass in the old version and new version: " +
...
glob
  ...
  .forEach(function copyMasterToStable(masterFilename) {
    ...
    fs.createReadStream(masterFilename).pipe(
      fs.createWriteStream(stableFilename)
    );
  });
