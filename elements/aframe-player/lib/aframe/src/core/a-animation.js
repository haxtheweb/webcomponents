    ...
    createdCallback: {
      value: function() {
        ...
        if (!hasLoggedDeprecation) {
          warn(
            "<a-animation> has been deprecated and will be replaced by the animation " +
    ...
    getTween: {
      value: function() {
        ...
        if (
          repeat === Infinity &&
          fill === FILLS.forwards &&
          [DIRECTIONS.alternate, DIRECTIONS.alternateReverse].indexOf(
            data.direction
          ) !== -1
        ) {
          yoyo = true;
    ...
    getDirection: {
      value: function(direction) {
        if (direction === DIRECTIONS.alternate) {
          this.prevDirection =
            this.prevDirection === DIRECTIONS.normal
              ? DIRECTIONS.reverse
              : DIRECTIONS.normal;
        ...
        if (direction === DIRECTIONS.alternateReverse) {
          this.prevDirection =
            this.prevDirection === DIRECTIONS.reverse
              ? DIRECTIONS.normal
              : DIRECTIONS.reverse;
          return this.prevDirection;
        }
...
function getAnimationValues(el, attribute, dataFrom, dataTo, currentValue) {
  ...
  function isColor() {
    ...
    return (
      schema && schema[propertyName] && schema[propertyName].type === "color"
    );
  }
