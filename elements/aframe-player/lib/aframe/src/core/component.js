...
Component.prototype = {
  ...
  updateProperties: function(attrValue, clobber) {
    ...
    skipTypeChecking =
      attrValue !== null &&
      typeof this.previousAttrValue === "object" &&
    ...
    if (!this.initialized) {
      ...
      oldData =
        !isSinglePropSchema ||
        typeof parseProperty(undefined, this.schema) === "object"
          ? {}
          : undefined;
      ...
      if (utils.deepEqual(this.oldData, this.data)) {
        return;
      }
      // Store current data as previous data for future updates.
      this.oldData = extendProperties({}, this.data, isSinglePropSchema);
  ...
  buildData: function(newData, clobber, silent, skipTypeChecking) {
    ...
    if (isSinglePropSchema) {
      ...
      // Apply defaults.
      for (
        i = 0, keys = Object.keys(schema), keysLength = keys.length;
        i < keysLength;
        i++
      ) {
        ...
        data[keys[i]] = isObjectOrArray(defaultValue)
...
function cloneData(data) {
  ...
  for (key in data) {
    ...
    clone[key] = isObjectOrArray(parsedProperty)
...
function extendProperties(dest, source, isSinglePropSchema) {
  if (isSinglePropSchema && (source === null || typeof source !== "object")) {
    return source;
  }
  var copy = utils.extend(dest, source);
  var key;
  for (key in copy) {
    if (!copy[key] || copy[key].constructor !== Object) {
      continue;
    }
